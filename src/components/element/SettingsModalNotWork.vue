...
export default {
    ...
    data() {
        return {
            activeTab: 'feed',
            nickname: this.$store.getters['nickname/getNickname'](this.pokemon.id),
            foodItems: [
                {
                    id: 1,
    ...
    methods: {
        ...
        saveNickname() {
            if (this.nickname.trim() === '') {
                this.nickname = this.pokemon.name;
            }
            this.$store.dispatch('nickname/setNickname', { pokemonId: this.pokemon.id, nickname: this.nickname });
        },
        removePokemon() {
            this.$store.dispatch('nickname/removeNickname', this.pokemon.id);
            this.$emit('remove', this.pokemon.id);
        },
        subtractMoney(price) {
